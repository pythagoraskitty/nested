...
def hex_to_dec(s):
    ...
    if '0' <= s[0] <= '9':
        ...
        dec += ord(s[0]) - ord('A') + 10
    dec *= 16
    if '0' <= s[1] <= '9':
        dec += int(s[1])
    elif 'A' <= s[1] <= 'F':
        dec += ord(s[1]) - ord('A') + 10
    return dec
...
def hex_to_rgb_str(hex_str):
    red = hex_to_prop(hex_str[1:3])
    green = hex_to_prop(hex_str[3:5])
    blue = hex_to_prop(hex_str[5:])
    return brace("\\definecolor", "color_" + hex_str[1:], "") + brace("", "rgb", "") + brace("", str(red)+","+str(green)+","+str(blue), "")  ###
...
def make_tex_str(scale, a, b, num_sq, color = "black", places = 3):
    ...
    data += brace("\\usepackage[usenames,dvipsnames]","color", "")
    data += brace("\\usepackage", "tikz", "\n\n")
    if color[0] == "#":
        data += hex_to_rgb_str(color)
